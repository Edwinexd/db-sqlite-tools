...
\author{
  Edwin Sundberg\\
  {\small\textcolor{darkgray}{\href{mailto:edwin@e-su.se}{edwin@e-su.se}}} \\
  \and
  Melle Carnesten\\
  {\small\textcolor{darkgray}{\href{mailto:melle.carnesten@dsv.su.se}{melle.carnesten@dsv.su.se}}}
}
...
If we are more interested in querying for a specific date the following query can be used:

\begin{lstlisting}[caption={Querying using date()}, label={lst:sqliteDate}]
SELECT
  *
FROM
  inboxes
WHERE
  date(createdAt) = '2025-01-01';
\end{lstlisting}
...
\begin{figure}[!htb]
  ...
  \caption{ERD of the implemented database model.}
